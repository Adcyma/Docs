...
The Adcyma API lets you manage Entra ID user lifecycles — create new users, update their attributes, look them up, and offboard them — directly from your own systems.

**Base URL**: `https://api.adcyma.com`  
**Path prefix**: `/v1/api`  
**Content type**: `application/json`

All endpoints described in this section are mounted under the path prefix above. For example, the create-user endpoint is reachable at `https://api.adcyma.com/v1/api/user`.

### Authentication

Every request (except the health check) must include three headers:

- **X-Company-ID**: Your tenant identifier. 3–64 characters; letters, digits, hyphens and underscores only.
- **X-Api-User**: The username of an active API user from your tenant.
- **X-Api-Key**: The secret key paired with that API user.

API users and keys are managed in the **System Users** section of your Adcyma portal. The key is shown to you once, at creation time — store it somewhere safe. If you lose it, generate a new one and revoke the old.
...
Successful responses always wrap the payload in a `data` object:
...
Errors always come back in a single, predictable shape:
...
| `500`       | `INTERNAL_ERROR`        | Unexpected server-side error. Safe to retry after a brief backoff.           |
...
A single unauthenticated endpoint is exposed for uptime monitoring.
